...
Use these sections, in this order. Omit one only if it genuinely doesn't apply to the repo; never reorder.

The two **non-negotiable** rules are the [standalone rule](#the-standalone-rule) and the [footer](#the-footer--one-link-routed-by-visibility). Section *names* are fit-for-purpose: an app or tool uses **Installation**; a library or Nix module may name its getting-started section for its nature (e.g. **What it manages**, **What's in this flake**) — as long as both a getting-started section and a **Usage** section exist. A repo's `.readme-validator.yaml` is the per-repo enforcement of exactly which headings it requires; repos without one fall back to the default (`Installation`, `Usage`).
