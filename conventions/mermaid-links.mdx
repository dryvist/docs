...
export, no JavaScript needed.

### When to add clicks

Add `click` whenever a node represents a topic that has its own
detail page on this site, or its own canonical external URL. Examples
are a repo, a dashboard, or a vendor doc. This is **the default for
context, overview, and hub diagrams**, not an optional embellishment.

Skip clicks in two cases. First, nodes that don't map to a real
destination (a transient process, an abstract concept like "Code" or
"Ship"). Second, destinations that are still TODO and would 404.
...
The diagram disappears, and the back button is the only way home.
...
When the Mermaid bugs are fixed upstream, this convention expands to
allow external clicks; until then, internal-only.
...
- **Pure sequence / timeline diagrams** where every node is a stage
  in one flow (for example, CI step "Lint"). Clicks would imply each
  stage has its own page when the whole pipeline is one page.
