...
Pull requests are the unit of change that lands on the repo's integration branch — `develop` on git-flow repos, `main` on trunk-flow repos. See [Branch conventions → Which model applies](/conventions/branch-conventions#which-model-applies) to check which one a given repo uses. The title, the description, and the merge strategy together decide what the changelog says and how easily someone six months from now can answer "why did this happen?".
...
2. **`## Test plan`** — bulleted markdown checklist. What CI runs automatically (`bun run build`, `pnpm test`, `tofu fmt -check`), and what a reviewer still has to verify by hand (browser smoke test, reviewed Terrakube plan, anything CI cannot see). Tick the boxes as work completes.
...
- **`develop` → `main` promotion is merge-commit only.** Squash and rebase into `main` are banned, no exceptions — that includes the `release-please` release PR, which merge-commits like every other `main` PR.
- **`hotfix/*` is the one exception to PR targeting.** It branches from `main`, its PR targets `main` directly with a merge commit (never squash), and `main` is immediately back-merged into `develop` afterward.
