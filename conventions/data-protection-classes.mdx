...
Every data store in the homelab, including databases, config, object storage,
and ZFS pools, is assigned one of four protection classes. There is no fifth
class and no per-store exception outside the exemption tier below.
...
2. **Does anything outside the codebase, such as a human, an external API,
   or a sensor, write to it such that the last hour cannot be recreated?** →
...
Every class assignment reduces to one question. Is this store a **source of
truth**, meaning something writes to it that cannot be recreated? Or is it
**derived**, meaning an index, cache, or rebuild target some other source of
truth already covers? That single distinction collapses dozens of
...
<CardGroup>
  ...
  A worked P0 example: replication and layered backups for a shared application database.
</Card>
  <Card
    title="ZFS backup and replication"
    href="/infrastructure/zfs-backup-replication">
  A worked example at the host layer: the 3-2-1 rule built as four escalating ZFS layers.
</Card>
</CardGroup>
