...
%% Shape: parallel convergence. Boundary crossings: 0. Ranks: max 3.
%% Aspect: ~3.5:1 (LR). Pass.
flowchart LR
  Develop(["develop"])
  Feature(["feature/&lt;issue&gt;-slug"])
  PRDev{"PR to develop"}
  Promote{"Promotion PR"}
  Main(["main"])
  Hotfix(["hotfix/slug"])

  Develop --> Feature --> PRDev --> Develop
  Develop --> Promote --> Main
  Main --> Hotfix --> Main
  Hotfix -.-> Develop
  ...
  class Develop hop
  class Feature hop
  class PRDev,Promote gate
  class Main sink
  class Hotfix hop

  linkStyle 0,1,2,3,4,5,6 stroke:#4FB3A9,stroke-width:2px;
```
...
git-flow repos follow the model implemented by [git-flow-next](https://github.com/gittower/git-flow-next).
The `git flow` command-line tool is installed and configured globally (nix-managed `gitflow.*` git config).
...
Every merge into `main` produces a release: release-please watches `main`, opens the release PR
against it, and tags on merge. The release PR merge-commits like any other `main` PR. It is not a
squash exception. See [PR conventions](/conventions/pr-conventions) for the full merge-strategy
breakdown.
...
never rebase). Each promotion merge is a release.
...
2. Start the branch there with `git flow feature start <name>`. `git-flow-next` prepends the `feature/`
   prefix itself, so `<name>` is the slug only. `git flow feature start 123-fix-inventory-loader`
   produces `feature/123-fix-inventory-loader`; passing the prefix yourself doubles it. Include the
   GitHub issue number when one exists. Never invent an issue number; use a short descriptive slug
   for issue-less maintenance.
...
4. Open the PR against `develop`. Squash-merge ordinary feature PRs. Use a merge commit into
   ...
   multi-commit changes, and release or hotfix back-merges.
5. `hotfix/*` is the one exception to PR targeting: it branches from `main`, its PR targets `main`
   directly with a merge commit (never squash), and `main` is immediately back-merged into `develop`
   afterward.
...
- `git flow finish` detects the branch type from any topic branch. Prefer finishing through the PR
  flow in the preceding steps; use `finish` directly only where a PR is overkill (for example,
  local-only cleanup) and the branch target allows direct pushes.
...
Repos without a `develop` branch use trunk-based flow: no `feature/*`, `release/*`, or `hotfix/*`
git-flow types, and PRs squash-merge straight into `main` (see [PR
...
On a trunk-flow repo, the long forms (`feature`, `bugfix`) don't appear in any Conventional Commits
...
Its own `release/` and `hotfix/` types are distinct from git-flow-next's identically named branch
categories in the preceding sections. Trunk-flow repos use neither; git-flow repos use git-flow-next's.
