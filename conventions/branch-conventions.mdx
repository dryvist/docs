...
Branch slashes survive as directory separators when this org's bare-repo + worktree layout is used. `feat/add-x` becomes `<repo>/feat/add-x/`. That makes `feat/`, `fix/`, etc. literal directories on disk — another reason to use the short types: `feature/` would mean five extra characters in every working path forever.
