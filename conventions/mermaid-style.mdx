---
title: "Mermaid style guide"
description: "Canonical, byte-for-byte Mermaid authoring rules for docs.jacobpevans.com: theme directive, shape vocabulary, classDef and linkStyle palettes, the four narrative shapes, and density caps."
page_type: "reference"
---
{/* projected from the authoring site; do not edit here */}
The detailed style and layout rules for authoring diagrams on this site. For the
reader-facing summary and when to draw a diagram at all, see
[`conventions/diagramming`](/conventions/diagramming); for making diagram nodes
navigable, see [`conventions/mermaid-links`](/conventions/mermaid-links).
...
represents; don't invent new classes per page. This keeps the same color
...
with the page; the **border color** is what carries the semantic
...
- **Mix node shapes** purposefully. Use stadiums for processes, cylinders
...
- **PARALLEL CONVERGENCE**: two chains that join at the end. Use
  ...
  in source). ELK ranks the shorter chain on the second row.
...
- **Allowed** subgraph names, for actual co-location: `Proxmox cluster`,
...
- **Never emit a global `linkStyle default stroke:#E06B4A`**. It paints
  every edge the same color and throws away semantic differentiation.
...
| Mermaid | One of the four preceding shapes; structure ≥ 4 nodes with real edges |
