...
- **Single update propagates everywhere.** `nix flake update` in the consumer (or in nix-devenv for cross-org propagation) pulls every hook to a new pinned version. No per-repo `rev:` bumps. (Non-Nix consumers instead get Renovate `rev:` bumps, scheduled and auto-merged by trust tier — see [Dependency automation](/infrastructure/cicd/dependency-automation).)
