...
- **`S3_INVENTORY_KEY` selects which environment's input to read** (see [Per-environment inputs](#per-environment-inputs)). It is set explicitly per run; if unset, it defaults to the production object.
...
- **Never `git add deployment.json`.** The repo keeps only `deployment.json.example` as a shape reference; the live file is gitignored by design. The develop-environment object follows the same rule — the repo keeps only `deployment.develop.json.example`.
...
The ACID contract above holds **per environment**. `tofu-proxmox` now has two independent inputs — a production object and a develop object — so a `develop` branch can plan and apply real infrastructure without contending with production over one state. Each is a separate versioned object with its own tfstate, and each obeys the whole contract independently.

**Selection is explicit.** An operator or CI job sets `S3_INVENTORY_KEY` to choose which object a run reads — there is no inference from the git branch name. Unset, it defaults to the production object. The fetched object then declares its own identity in an `environment` field, and Terragrunt derives three keys from it:

| Derived key | Production (`homelab`) | Develop (`develop`) |
| --- | --- | --- |
| tfstate | *(current literal — unchanged)* | nested under a `develop/` prefix |
| input mirror (DR fallback) | *(current literal — unchanged)* | nested under a `develop/` prefix |
| published inventory | *(current literal — unchanged)* | nested under a `develop/` prefix |

Three properties make this safe:

- **Production paths are pinned.** The production branch of each derivation is the exact pre-existing literal, so adding the develop environment changes nothing a production plan touches — no state migration, no re-published inventory. This is the load-bearing invariant: any drift in a production key would make the next production plan initialize a fresh empty state and propose a full destroy.
- **Isolation by construction.** Separate tfstate keys mean separate state locks (the lock id is per-key), so the two environments never collide in state. Guests are separated on the cluster by an environment digit in their id and a dedicated non-production network segment.
- **One writer across both.** The environments share one physical cluster, so the [global flow lease](#the-global-flow-lease--one-mutating-flow-at-a-time) still serializes mutating flows across environments — one apply at a time, regardless of which environment it targets.

An unknown `environment` value fails loud before any plan, the same way a missing input does — the allowlist is part of the contract, not a convenience.
...
<CardGroup cols={2}>
  ...
  <Card title="OpenTofu on AWS" icon="lock" href="/infrastructure/tofu-aws/overview">
