...
- **`S3_INVENTORY_KEY` selects which environment's input to read** (see [Per-environment inputs](#per-environment-inputs)). It is set explicitly per run; unset it defaults to the production object.
...
- **Never `git add deployment.json`.** The repo keeps only `deployment.json.example` as a shape reference; the live file is gitignored by design. The develop-environment object follows the same rule — repo keeps only `deployment.develop.json.example`.
