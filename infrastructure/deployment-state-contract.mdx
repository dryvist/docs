...
The requirement this pushes onto the object store is exact: **it must implement `If-None-Match` conditional writes.** Not every S3-compatible implementation does.

- **AWS S3** and **MinIO** support conditional writes and can hold a `use_lockfile` lock.
- **RustFS** (alpha) does **not** yet honor `If-None-Match` correctly ([rustfs#1458](https://github.com/rustfs/rustfs/issues/1458)), so it silently fails to serialize writers and **cannot** be used as a lock-bearing backend until fixed.

Migrating off a legacy DynamoDB lock table follows the "run both, then drop DynamoDB" burn-in OpenTofu documents: hold `use_lockfile` and the DynamoDB table together for a period, then drop the DynamoDB leg (a backend re-init, sequenced as its own change). The target end-state is `use_lockfile` only; `tofu-unifi` already runs lockfile-only.
