...
A worked example: a media service (Tier 7) that is user-facing (sub-tier `0`), normal criticality (`5`), running as an LXC (`0`), first instance (`0`), in production (`0`) sorts as `750000`. Its tier digit is `7`, so it lives on VLAN `70`. Identity read off, network derived — same number.
