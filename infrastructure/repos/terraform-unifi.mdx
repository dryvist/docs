...
- Subnet values stay in the secret store and are consumed by both `tofu-unifi` and `terraform-proxmox` — single source, no drift. Passphrases, VPN keys, and shared secrets live in SOPS, never in plaintext config.
