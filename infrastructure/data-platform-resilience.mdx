...
Both copies are live. The database ships every write to the on-site store
continuously and takes a full base backup weekly with incrementals in between, so
a restore can land on any point in time rather than the last nightly snapshot. The
off-site copy is byte-for-byte identical to the on-site one — same layout, same
names — so a restore reads the same paths either way and only swaps which store it
points at.

<Note>
As-built pending: the scheduled restore drills. Until a restore is rehearsed, the
tier is unproven.
</Note>
