...
[OrbStack](https://orbstack.dev/) provides a Kubernetes cluster on macOS that the
[`orbstack-kubernetes`](/infrastructure/repos/orbstack-kubernetes) manifest set deploys workloads
into. There is no Kubernetes anywhere else on this stack. The homelab Proxmox cluster runs [LXC +
Ansible](/infrastructure/lxc-vs-docker) for the equivalent role. This page explains why the split
exists and what runs where.
...
Five StatefulSets in a single `monitoring` namespace:
...
`cribl-mcp-server` (Cribl Cloud's MCP API surface) sits alongside but carries no pipeline traffic, so it's off the preceding flow.
...
- The macOS host telemetry pack
  ([`cc-edge-the-mac-pack`](/observability/repos/cc-edge-the-mac-pack)) needs a **native
  macOS Cribl Edge install**, not a container, because its exec inputs call `powermetrics`, `pmset`,
  `ioreg`, `memory_pressure`.
...
The architecture rule for the cluster is mechanical: `cribl-edge-standalone` sends only to
`cribl-stream-standalone` on HEC port 8088. Edge does not talk directly to Splunk. Stream is the
sole component with Splunk egress. Network policies in the manifest set enforce this; no one can
shortcut it.
...
Secrets are pre-injected into the Claude Code session via Nix + direnv (SOPS-decrypted env vars).
`secrets.enc.yaml` is the source of truth. Base manifests in `k8s/monitoring/` use the literal
`PLACEHOLDER_HOME_DIR` for hostPath volumes; it is never replaced in the base. The generated
`k8s/overlays/local/` is gitignored and produced at deploy time. `make deploy-doppler` does the
whole dance; CI enforces the same chain on a self-hosted ARM64 runner.

## When to add a new workload

The choice between K8s, LXC, and Docker comes down to two questions: what platform is the host,
and does the vendor leave a native path? On macOS, K8s on OrbStack is the default. On Proxmox, LXC
is the default. On either host, Docker is the answer only when the vendor leaves no other option.
See the [LXC vs Docker decision tree](/infrastructure/lxc-vs-docker) for the full logic.
