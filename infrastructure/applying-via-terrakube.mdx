...
flowchart LR
  Op([Operator or agent<br/>tofu apply])
  TK([Terrakube workspace<br/>queues the run])
  Bao[(OpenBao<br/>short-lived token)]
  Prov([Provider API<br/>Proxmox / AWS / GitHub])
  State[(Workspace state<br/>versioned)]
  ...
  classDef actor    fill:#102937,stroke:#4FB3A9,stroke-width:2px,color:#F4EFE6;
  ...
  class Op actor
  class TK,Bao control
  class Prov,State store

  linkStyle default stroke:#4FB3A9,stroke-width:1.5px;
```

The operator never holds a provider credential. Terrakube mints one from OpenBao per run and
...
1. **Authenticate once per machine.**
   ...
   Some workstations only get `tofu` through per-repo dev shells, with no
   global install. On such a machine, run it in an ephemeral shell instead:
...
- Granting a new operator or agent access means minting a token for the
  team they belong to. There is no shared static secret and no keychain
  entry to distribute.
