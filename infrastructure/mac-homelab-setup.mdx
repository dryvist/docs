...
sudo tcpdump -i en0 -v -s1500 "udp port 67 or udp port 68" > /tmp/dhcp.txt 2>&1 &
sudo ipconfig set en0 NONE; sudo ipconfig set en0 DHCP; sleep 4
...
## Enforce it across every Mac

Toggling this by hand on each machine and each network does not scale, and the
default flips back on every new network. The homelab enforces the setting
declaratively in [nix-darwin](/nix/nix-darwin): a module takes a list of
trusted network names and holds Private Wi-Fi Address off for those networks on
every Mac, so a new machine or a rejoined network never silently reverts to
`Mac`.
