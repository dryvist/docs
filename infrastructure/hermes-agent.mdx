...
- **Brain:** an always-on local GPU model (OpenAI-compatible), so the agent never
  depends on an external API or a sleeping laptop. The brain is a **router alias**
  so the physical model can change without reconfiguring the agent; it currently
  resolves to a mixed-precision 4-bit Qwen3.6-35B-A3B (OptiQ) selected by the
  [agentic tool-calling benchmark](/tools/mlx-benchmarks) as the top multi-turn
  tool-caller. **Thinking stays ON** for this brain — with reasoning disabled the
  same model degrades to plain-text pseudo tool-calls after a handful of turns.
...
- **Splunk MCP**: The `mcp_servers.splunk` entry connects to the Splunk MCP Server app (Splunkbase 7931) at `${SPLUNK_MCP_URL}` (which MUST be the management base plus the `/services/mcp` path) using `Authorization: Bearer ${SPLUNK_MCP_TOKEN}`. Tokens are not generic Splunk JWTs; the app strictly accepts tokens minted by its own `GET /services/mcp_token?username=<user>&expires_on=+90d` endpoint. These minted tokens have an `mcp` audience, are RSA-encrypted, and enforce encryption by default (`require_encrypted_token`). The dedicated service user's token is minted automatically and delivered to the agent via the shared secrets engine.
