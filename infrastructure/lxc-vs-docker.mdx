...
2. **Is there a single binary or a native package?** If yes: LXC + an Ansible role under [`ansible-proxmox-apps`](/infrastructure/ansible-proxmox-apps). Default path.
...
LXC also matches the operational model. [`terraform-proxmox`](/infrastructure/terraform-proxmox) provisions LXC and VMs from a single Terragrunt apply. [`ansible-proxmox`](/infrastructure/ansible-proxmox) configures hosts. [`ansible-proxmox-apps`](/infrastructure/ansible-proxmox-apps) lands the workload. The triad is the production path; everything that lives in this triad gets re-deployed on the same cadence and observed on the same dashboards.
