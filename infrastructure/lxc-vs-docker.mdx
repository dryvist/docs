...
LXC also matches the operational model. [`tofu-proxmox`](/infrastructure/repos/tofu-proxmox) provisions LXC and VMs from one Terrakube-managed OpenTofu workspace. [`ansible-proxmox`](/infrastructure/repos/ansible-proxmox) configures hosts. [`ansible-proxmox-apps`](/infrastructure/repos/ansible-proxmox-apps) lands the workload. The triad is the production path; everything that lives in this triad gets re-deployed on the same cadence and observed on the same dashboards.
