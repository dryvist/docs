...
An IaC repo on git-flow (default branch `develop`) instead follows [Branch conventions](/conventions/branch-conventions#git-flow-branch-types-repos-with-develop): `main` requires merge commits from `develop` (squash and rebase banned there), and the rules above apply to `develop` as its protected integration branch.
