...
<Steps>
  ...
  <Step title="Supply credentials via aws-vault + Doppler">
    Profile is `tf-runs-on`; Doppler config is inherited from the infrastructure project's `prd` config. `RUNSON_LICENSE_KEY` is mapped into `license_key` via `terragrunt.hcl`.
  </Step>
