...
flowchart LR
  Update{Renovate<br/>update opens}
  MinorPatch([Minor / patch<br/>any tier])
  Major([Major])
  Auto([Soak 0-3 days<br/>then auto-merge])
  FirstParty([First-party<br/>auto-merge immediately])
  Review([dep:review PR<br/>for a human])

  Update -->|minor/patch| MinorPatch --> Auto
  Update -->|major| Major
  Major -->|first-party| FirstParty
  Major -->|trusted/untrusted| Review

  classDef gate fill:#102937,stroke:#F4EFE6,stroke-width:2px,color:#F4EFE6;
  classDef auto fill:#102937,stroke:#4FB3A9,stroke-width:2px,color:#F4EFE6;
  classDef human fill:#102937,stroke:#E06B4A,stroke-width:2px,color:#F4EFE6;

  class Update gate
  class MinorPatch,Auto,FirstParty auto
  class Major,Review human
