...
See [Dependency automation](/infrastructure/cicd/dependency-automation) for the full trust-tier, scheduling, automerge, and version-pinning policy — including which action sources ride `@main` and which are SHA-pinned.
...
Third-party actions — trusted or untrusted — are unaffected and stay SHA-pinned, each with its released version tag as a trailing comment. The full per-scanner posture lives in [`dryvist/.github` → SECURITY.md](https://github.com/dryvist/.github/blob/main/SECURITY.md).
