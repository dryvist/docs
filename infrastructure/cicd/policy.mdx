...
| --- | --- |
| Self-references (`dryvist/*`, `JacobPEvans/*`) | `@main` — never a SHA or version tag. The scanners allow this; see [scanner posture](#scanner-posture-for-self-references) below |
...
- **CodeQL** — native code scanning via the org Security Configuration (public repos only — free; private repos excluded to avoid the paid Code Security charge). No committed config: default setup ignores query-filters, and same-org `@main` false positives ([github/codeql#18316](https://github.com/github/codeql/issues/18316)) are dismissed in the code scanning UI.
