...
[`terraform-runs-on`](/infrastructure/terraform-runs-on) uses the same mechanism but keeps the encrypted file smaller — it leans on Doppler for AWS creds and reserves SOPS for bootstrap config that doesn't change.
