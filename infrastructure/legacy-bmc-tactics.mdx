...
{/*Shape: decision tree, pick a pattern by need. 6 nodes. Aspect ~3:1 LR.*/}
...
flowchart LR
  Need{What do you<br/>need to do?}
  Monitor([Just watch the screen])
  Interact([Click / type into<br/>BIOS or GRUB])
  Install([Install an OS,<br/>no VGA at all])
  P1([Pattern 1<br/>HTTP screenshot API])
  P2([Pattern 2<br/>wrapped Java applet])
  P34([Pattern 3/4<br/>USB or PXE boot])
  ...
  classDef path     fill:#102937,stroke:#4FB3A9,stroke-width:2px,color:#F4EFE6;

  class Need decision
  class Monitor,Interact,Install,P1,P2,P34 path

  linkStyle default stroke:#4FB3A9,stroke-width:1.5px;
```

Pattern 1 covers most needs at the lowest cost. Reach for Pattern 2 or 3/4 only when Pattern 1
can't do the job.
