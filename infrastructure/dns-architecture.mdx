...
what makes the [DNS-first addressing model](/infrastructure/vmid-network-tiers)
...
- **DNS-over-HTTPS (DoH).** The resolver-to-upstream hop is encrypted, so
  ...
  so the resolver dials it directly instead of resolving the DoH hostname
...
- [Self-hosted ChatGPT](/infrastructure/local-llm) — an example of a service
