...
The full family → port → index → sourcetype table lives on the [observability overview](/observability/overview#syslog-ingest-families). Hypervisor firewall drops ride the `linux` family: the node firewall logs to a file rather than the journal, so each node's rsyslog tails it with `imfile` and forwards it on the same port — searchable as `index=os process=pve-firewall`.
...
The same Edge also ships the **Mac firewall unified-log**: a launch daemon tails the firewall subsystems (application firewall, network extension, packet filter) as ndjson into a rotated file, and a file input forwards it over the shared S2S port with `index=firewall` stamped at the source. This replaced the retired BSD-syslogd remote forward, which could never be skew-safe (RFC3164 carries no year and no timezone, and workstations deliberately keep local time).
