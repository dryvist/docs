...
description: "One diagram per concern: syslog families, AI command-line shipping, the local LLM stack, OTLP fan-out, NetFlow, and HEC fan-in."
...
overview](/observability/overview#syslog-ingest-families).

Hypervisor firewall drops ride the `linux` family. The node firewall logs to
...
backend directly. Edge fans traces out to Langfuse and [Phoenix](/observability/phoenix) — never
Splunk, which does not fit the trace format. See [Data pipelines](/architecture/data-pipelines#fan-out-every-source-to-every-destination)
for the fan-out default this follows.
