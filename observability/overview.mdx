...
Each index also has a **silence detector** — a per-index alert that fires when the index stops receiving events within its expected cadence. A broken port, token, or sender surfaces as "index went quiet" instead of a silent gap discovered weeks later.
