...
| Zone | What's there |
| --- | --- |
| **External** | Internet ingress over WireGuard; AWS holds the DR footprint (Splunk failover, Bedrock, S3) |
| **Networking** | UniFi gateway + switches + APs, 10G fibre backbone where it matters |
| **Proxmox cluster** | HAProxy LXC, Cribl Edge LXCs, Splunk indexer + search heads, Home Assistant LXC, dedicated docker-host VM for vendor-only images |
| **Personal devices** | Mac (`nix-darwin`), phone (Home Assistant app) |

Data flow: UniFi feeds Proxmox; HAProxy fronts Cribl Edge; Cribl forwards over HEC to Splunk; Splunk replicates a DR slice to AWS.
