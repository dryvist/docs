...
| Tool-call parser | model-appropriate (e.g. `hermes` for Qwen) | The parser must match the model's chat template or tool calls silently break. See [Backends & tool calling](/local-llm/backends). |
