...
A second 128 GB Mac is coming. The goal is overnight, latency-tolerant batch
work that's bigger or better than a single 128 GB machine can do alone. This page
is the honest version of how that works — because the obvious mental model is
wrong in a way that changes the whole design.
...
The fast interconnect is the real, recent enabler: **RDMA (remote direct memory
access) over Thunderbolt 5**, exposed to MLX through Apple's `mlx.distributed`
communication backends. "Direct memory access between the machines" is exactly
the right description of the *link* — it just moves bytes between two separate
pools, it doesn't fuse them.

| Path | Rough bandwidth | vs on-chip |
| --- | --- | --- |
| One M4 Max, unified memory (on-chip) | hundreds of GB/s | 1× |
| RDMA over Thunderbolt 5 (real, measured) | a few GB/s | ~1/100–1/150 |
...
- RDMA needs explicit one-time enablement, and the Thunderbolt bridge can reset
  across reboots and OS updates — a job that ran last week may need re-enabling.
...
Thunderbolt RDMA path works on *this* hardware, baseline a single Mac, compare
