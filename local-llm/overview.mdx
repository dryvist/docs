...
routes to whichever model answers. Model serving spans a workstation-class
machine for larger models and a dedicated GPU host for smaller ones, with the
gateway selecting between them automatically — no client needs to know which
one served the request.

The homelab's self-hosted autonomous agents run continuously and connect to
that same endpoint as their model backend, alongside interactive clients like
the browser and terminal tools described above.
...
The second 128 GB Mac has arrived, and it is the always-on large-tier host —
live, serving two models resident concurrently with measured throughput (see
[Homelab GPU](/local-llm/homelab-gpu)). It does **not** turn two machines into
one 256 GB pool — Apple Silicon unified memory can't be merged across a cable.
