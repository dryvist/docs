---
title: "Local LLM"
description: "One strategy for local inference: a fast resident model on each client laptop, and an always-on desktop-class Apple Silicon host serving the large tier to the whole LAN. Laptops are the clients."
page_type: "overview"
---
...
Antigravity, the routines) lives above them and calls in over HTTP.

- **The client laptops**: `mlx-lm` workers behind `llama-swap` on a
...
smaller, faster tier. The gateway selects between them automatically, so no
client needs to know which one served the request. This GPU host is
distinct from the older dedicated-GPU approach for large models noted earlier,
...
A free hosted tier sits behind both local tiers in the fallback order. It is
reached only when local capacity is unavailable, never in front of it. See
[Model routing](/ai-development/model-routing#delegation-the-cheapest-capable-tier)
...
It is the intelligence tier: it optimises for answer quality, not throughput.
A boot warmup LaunchAgent (`mlx-warmup`) faults model weights into memory at
boot so the first request does not pay the cold-start cost.

For what that host actually loads right now, read
[What is actually loaded](/local-llm/mac-studio#what-is-actually-loaded), a
...
[Re-measured in isolation](/local-llm/mac-studio#re-measured-in-isolation).
...
<Warning>
  **Why those figures were pulled, and why you should distrust any number that
  lacks its provenance**. The swapping proxy once served the resident model's
  weights under every other model's label. Single-model collapse grafted each
  inactive model's *physical identifier* onto the surviving entry as an alias,
  which inflated one measurement by 7x. That defect is fixed: an inactive
  model's physical id no longer gets aliased, and now returns HTTP 404. But
  figures measured while it was live remain provisional regardless. The
  concurrency aggregate is not a general
  result either. On a large MoE model, four streams measured *worse* than one
  (verified 2026-07-26/27). Read
  [Verifying the instrument](/local-llm/verifying-the-instrument) before citing
  any number on this site.

  They were also quoted on the wrong metric, and without a regime. The headline
  figure is **cumulative** tokens/second: prompt plus completion over total
  wall-clock, because decode-only rates hide real prefill gains. But cumulative
  is meaningless without its prompt size, so both must be published together.
  See [the metric definition](/tools/mlx-benchmarks#the-headline-metric-is-cumulative-not-decode-only),
  [why the regime matters](/tools/mlx-benchmarks#cumulative-is-meaningless-without-its-regime),
  and the [re-measured figures](/local-llm/mac-studio#re-measured-in-isolation).
</Warning>
...
[Distributed & multi-Mac](/local-llm/distributed) for the honest picture.
...
- **A small resident set, not a rotation.** Swapping a multi-GB model evicts
  ...
  aliases (`default`, `coding`, `quickest`, `tool-calling`, …) resolve onto that
  resident set, so a role never triggers a swap. How many are resident is a
  property of the host, read from the registry, not a fixed number.
...
- **Measure, don't claim; then verify the instrument.** No tuning change ships
  on a marketing number, only a measured one. But a measurement is only as good
  as proof of which weights the process loaded, so every number carries that
  provenance. See [Verifying the instrument](/local-llm/verifying-the-instrument).
- **MoE for throughput.** A sparse mixture-of-experts model with a few billion
  ...
  size. That's the lever that makes a big model usable on a laptop.
