...
| --- | --- | --- | --- |
| **`Qwen3.6-35B-A3B-OptiQ-4bit`** | ~20 GB (Resident) | Resident Tool-Calling Brain | **The agentic tool-calling brain.** Selected by the agentic tool-calling benchmark: 100% valid tool calls and a clean 20-round multi-turn track. **Runs with thinking ON** — required, since the same weights degrade to plain-text pseudo tool-calls around round 6 with thinking off. Serves the Hermes agent behind its router alias. |
