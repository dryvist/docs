...
| **`Qwen3-Next-80B-A3B-Thinking-4bit`** | ~45 GB (On-demand) | Large-phase brain (`ai-default` 00:00 UTC) | **The larger reasoning brain in the daily rotation.** Loaded on demand only while `ai-default` maps to it; a first-class router alias. Thinking-native. **Give it a generous token budget:** at a small `max_tokens` its verdict truncates mid-reasoning (`finish_reason: length`) — a token-budget limit, not a tool-format failure. At an 8K budget it runs a clean 20-round track and carries the same `repetition_penalty` (1.05) guardrail as the resident brain. See [`docs/BRAIN_ROTATION.md`](https://github.com/dryvist/ansible-proxmox-apps/blob/main/docs/BRAIN_ROTATION.md). |
