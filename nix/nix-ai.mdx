...
Runtime secrets are never stored in Nix store paths or committed files. Three injection patterns are in use:

| Pattern | Used by | Mechanism |
| --- | --- | --- |
| OpenBao subprocess wrapper | Splunk MCP | Reads the [`ai-readonly` AppRole credentials](/security/tools/openbao) from a dedicated macOS keychain, obtains a short-lived token, and injects the shared connection only into the child MCP process |
| Doppler subprocess wrapper | Google Workspace MCP | `doppler run -p <project> -c prd -- <binary>` |
| macOS Keychain via shell init | HuggingFace token, service API keys | `security find-generic-password` exported in `~/.zshrc` |
