...
| --- | --- | --- |
| OpenBao subprocess wrapper | Splunk MCP | Reads the `ai-readonly` AppRole from a dedicated macOS keychain, obtains a short-lived token, and injects the shared connection only into the child MCP process |
...
same current Splunk connection through its machine AppRole. Terrakube workload
