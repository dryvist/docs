...
description: "Cross-platform user environment via home-manager: shell, git, editor, tmux, and every command-line tool that lives in $HOME."
page_type: "repo"
---
{/* projected from the authoring site; do not edit here */}
import { RepoMeta } from "/snippets/repo-summary.mdx";
...
`nix-home` is the home-manager layer. It owns `$HOME`: the user's shell, dotfiles, git config, tmux, and VS Code. Every command-line tool belongs here before any project touches a thing.

## What it does

- Declares `home.packages`: user-level command-line tools that follow `$HOME`, not the system
...
- Pulls AI tooling from [`nix-ai`](/nix/nix-ai) when AI command-line tools should live in the user PATH
