...
description: "Six surfaces, one stack: how each one depends on the others."
...
<Card
  ...
  icon="robot"
  href="/ai-development/overview">
  Claude · Antigravity · Copilot · MLX
</Card>
<Card
  title="Dev Tools"
...
%%{init:
  ...
  click Cfg "/infrastructure/configuration/overview" "Ansible-managed configuration"
  click Obs "/observability/overview" "Cribl · Splunk · OTEL"
  click Nix "/nix/overview" "macOS host · user env · dev shells"
  click AI "/ai-development/overview" "Claude · Antigravity · Copilot · MLX"
...
Two chains converge on Observability: the homelab chain (Infrastructure → Configuration) and the dev
chain (Nix → AI Development). Security (coral) gates the upstream end of each: credentials, branch
protection, isolation. Dotted amber edges are control, not data.
...
<Card
  title="Security · cross-tool flows"
