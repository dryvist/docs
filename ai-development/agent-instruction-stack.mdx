...
- **Always-on core.** One file, loaded every session, holds only the rules that shape
  *every* task — how to verify work, when to ask before acting, how to tell hard truths.
  ...
  tier.
- **Path-scoped rules.** A rule that only matters when you touch a certain kind of file
...
operations agent, and a chat surface should think the same way but know different things
about who they are and what they can reach.
...
— the extra tools, the tighter autonomy, the surface-specific scope — never a full copy.
A variant is a diff, not a fork. When the shared behavior changes, you edit the base once
and every surface inherits it; the variants stay small because they carry only their own
exceptions.

## A size gate keeps the core honest

A character budget that nothing enforces drifts upward one "just this once" at a time. Put
the budget in CI. A small check counts the always-on core and the base prompt block, and
...
always-on context bloat silently. Pair the gate with an evaluation scaffold — a small set
of probe tasks you can re-run — so a change to the core is judged by whether it helps the
model, not only by whether it fits.
