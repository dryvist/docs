...
The default development loop is intentionally lightweight: gather enough context, choose the simplest verifiable path,
implement the smallest correct change, and report only the result plus verification. PRD-heavy, test-first gating
remains available for high-risk or explicitly gated work, but it is no longer the default for routine tasks.
