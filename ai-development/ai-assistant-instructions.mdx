...
- Owns the git-flow branching/merge model and `git-flow-next` tool usage — [`agentsmd/rules/git-flow.md`](https://github.com/JacobPEvans/ai-assistant-instructions/blob/main/agentsmd/rules/git-flow.md); the human-readable version lives in [Branch conventions](/conventions/branch-conventions) and [PR conventions](/conventions/pr-conventions)
