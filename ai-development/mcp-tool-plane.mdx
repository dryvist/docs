...
description: "Model Context Protocol servers give agents their tools. Registering them once behind a shared gateway, rather than per agent, is the same consolidation the model endpoint applies to inference, with the same failure mode when it is a single instance."
