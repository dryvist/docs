...
[`tofu-proxmox`](/infrastructure/repos/tofu-proxmox) is the canonical example: a `.sops.json` under each environment folder, listed in `.sops.yaml` with the age recipient that controls it. Terragrunt consumes it through the `sops_decrypt_file` data source; resolved values land in OpenTofu state encrypted by the state backend's KMS key, not on disk.

[`tofu-runs-on`](/infrastructure/cicd/tofu-runs-on) uses the same mechanism but keeps the encrypted file smaller — it leans on Doppler for AWS creds and reserves SOPS for bootstrap config that doesn't change.
