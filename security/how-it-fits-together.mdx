...
| **Local AI session** | `git`/`gh` → credential helper → secrets engine mints a scoped token → used in memory → nothing persists |
...
See [Local AI isolation](/security/local-ai-isolation) for the diagram and the full proof. Short version: no GitHub token sits on the workstation. A credential helper answers each `git` or `gh` request by minting a short-lived, scoped token; the token lives in memory for that operation and is never written to disk. Write access needs an explicit claim on one named repository.
