...
| **T2 — Runtime manager** | Self-hosted source of truth for live machine credentials and Terrakube dynamic provider credentials | OpenBao | AI and machines, via scoped native auth / signed SSH cert — no human in the loop |
...
| **macOS Keychain** | Service API keys no engine mints; BWS access token; Claude Code OAuth credential; human-only break-glass material | [macos-keychain](/security/tools/macos-keychain) |
