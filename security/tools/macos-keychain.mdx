...
Each `security find-generic-password` call against `elevate-access` triggers an unlock prompt. Inlining the read on every command would prompt every time. Fetch once, inject the variable across the session — and unset it when done.
