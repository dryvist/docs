...
That works at small scale. It scales poorly. OpenBao (the open-source fork of HashiCorp Vault from before the BSL relicense) is the planned answer: a single source of truth for cluster-local secrets, with leases, audit, and per-service auth methods.
