...
Agents provisioned with an OpenBao AppRole `role_id` and `secret_id` (whether injected via Doppler or read from the keychain) must exchange them for a short-lived `client_token` via the OpenBao API before reading secrets or running pipelines that require OpenBao data (such as the infrastructure inventory).
