...
<Steps>
  <Step title="Read references from disk">
    `bws_helper.py` loads `~/.config/bws/.env` — BWS secret IDs and keychain references only. No values here.
  </Step>
  <Step title="Fetch the BWS access token from the keychain">
    `security find-generic-password` retrieves `BWS_ACCESS_TOKEN` from `automation.keychain-db`. The token never reaches disk.
  </Step>
  <Step title="Call BWS">
