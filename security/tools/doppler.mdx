---
title: "Doppler"
description: "The T3 strict cloud tier — cloud-shared keys-to-the-kingdom under human approval, and the holder of secret-zero that bootstraps the T2 runtime manager."
page_type: reference
---
...
- Service API keys that only the macOS host uses (Keychain — see [macos-keychain](/security/tools/macos-keychain)).
- GitHub tokens — they are minted on demand and never stored (see [GitHub access](/autonomous-agents/github-access)).
...
| Locally selected AI project/config | `doppler run --` wrapper at subprocess launch | Local dev + the [`doppler-mcp`](/security/local-ai-isolation) bridge |
