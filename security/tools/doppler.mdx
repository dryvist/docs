...
The service token `GH_ACTION_DOPPLER_IAC_CONF_MGMT` is itself a Tier 1 secret distributed by `secrets-sync` to the two `_infra_repos`: `ansible-proxmox-apps` and `tofu-runs-on`.
