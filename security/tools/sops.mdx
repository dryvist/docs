...
It is **not** a vault — OpenBao (T2) is. SOPS is checked-in, encrypted-at-rest configuration for the narrow single-repo case: OpenTofu variables that name internal networks, ansible variables that vary per host but aren't truly secret-secret. Control-plane credentials (Proxmox/iDRAC/network) are **not** SOPS material — they live in OpenBao `secret/infra/*` (or, human-only, the Keychain).
