...
SOPS encrypts the *values* in a structured config file (YAML, JSON, TOML, dotenv, INI) while leaving the keys readable. The result is a `.sops.json` or `.sops.yaml` file that lives in git, decrypted at runtime by the age private key.
