...
**Bifrost** is the OpenAI-compatible HTTP gateway that sits between every AI tool on the workstation and whichever provider answers the call, routing requests to OpenAI, Gemini, OpenRouter, and local MLX based on task class. **PAL MCP** is the multi-model coordination layer on top of Bifrost, providing `clink` (parallel calls across models) and `consensus` (multi-model agreement verification) for complex coordination scenarios.

For configuration, model prefixing rules, offline mode, and usage patterns, see the dedicated pages: [Bifrost](/tools/bifrost) and [PAL MCP](/tools/pal-mcp).
