...
versions of a prompt: the one you use now and a candidate you want to try. It runs
...
Each answer is scored two ways. Exact checks run with no model in the loop (a number
matches, the output is valid JSON). Graded checks use a cheap grader model that
...
detail is missing, the model must ask or explain. It must not invent a call. A fabricated
...
OpenAI-family model, both through OpenRouter. A separate job can also run it
