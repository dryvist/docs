...
The pipeline on this repo, [`JacobPEvans/docs`](https://github.com/JacobPEvans/docs), uses six thin
caller files in `.github/workflows/` that delegate to reusable workflows in
[`JacobPEvans/ai-workflows`](https://github.com/JacobPEvans/ai-workflows). Each caller is 10–30
lines; the AI work all lives in the upstream reusable workflow.
...
- **`post-merge-docs-review.yml`** uses the [Post-Merge Dispatch
  Pattern](https://github.com/JacobPEvans/ai-workflows/blob/main/docs/PATTERNS.md#post-merge-dispatch-pattern).
  It is a two-job file because `push` events aren't supported by `claude-code-action@v1`, so the caller
  re-dispatches as `workflow_dispatch`.
...
Per [Git signing](/infrastructure/cicd/git-signing), every AI workflow mints a
`JacobPEvans-claude` installation token immediately before calling `claude-code-action@v1`, then
hands it in as `github_token` with `use_commit_signing: true`. Commits land web-flow-signed and
...
- **`issue-resolver.yml`**: `max_attempts: 1` per issue, `daily_limit: 5` per repo, and it skips any issue labeled `excluded_labels: "type:security,type:feature,type:breaking,size:l,size:xl"`
...
<Card
  title="ai-workflows getting started"
  icon="book"
  href="https://github.com/JacobPEvans/ai-workflows/blob/main/docs/GETTING_STARTED.md">
  Caller templates and the live workflow catalog.
</Card>
<Card
  title="Authentication"
  icon="key"
  href="https://github.com/JacobPEvans/ai-workflows/blob/main/docs/AUTHENTICATION.md">
