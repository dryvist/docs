...
Every reusable workflow uses [`anthropics/claude-code-action@v1`](https://github.com/anthropics/claude-code-action), authenticated via OpenRouter:

- `secrets.OPENROUTER_API_KEY` — the API key (set spend limits in the OpenRouter dashboard)
- `secrets.OPENROUTER_BASE_URL` — `https://openrouter.ai/api/v1`

OAuth-token auth (the Claude Code subscription) is **explicitly forbidden** in unattended CI — it violates the [Claude Code Terms of Service](https://www.anthropic.com/legal/terms). The full reasoning is in [AUTHENTICATION.md](https://github.com/JacobPEvans/ai-workflows/blob/main/docs/AUTHENTICATION.md).
...
<CardGroup cols={2}>
  ...
  <Card title="Authentication" icon="key" href="https://github.com/JacobPEvans/ai-workflows/blob/main/docs/AUTHENTICATION.md">
    OpenRouter setup, cost controls, alternatives, why not OAuth.
  </Card>
  ...
  <Card title="Secret distribution" icon="lock" href="/security/secrets-sync">
    How OPENROUTER_API_KEY and the App credentials land on each consumer repo.
  </Card>
</CardGroup>
