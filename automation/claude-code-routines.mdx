...
| --- | --- | --- |
| Morning Briefing | Daily 10:00 | Read-only activity summary: new PR reviews, opened issues, CI failures, anything that needs attention |
| The Sentinel | Daily 05:33 | Param/secret audit; picks one PR to review; flags operator-specific patterns |
| The Custodian | Daily 07:00 | Weighted-random maintenance: stale branch cleanup, missing labels, repo health audit |
| Issue Solver | Daily 00:00 + 12:00 CT | Picks one open issue, takes it from triage → draft PR in six phases |
| Daily Polish | Daily 04:00 | Deep-clean one repo per day: README gaps, missing `.gitignore`, CI config, test coverage |
| Weekly Scorecard | Mondays 10:00 | Portfolio health scores: repo count, open-issue median, CI pass rate, test coverage % |
