...
<Note>
Status: **in progress.** Core OpenBao is live and its AWS secrets engine now
brokers automation's AWS access; the GitHub service engine remains in rollout.
Each linked page carries the current status for its workstream.
</Note>
...
<Steps>
  ...
  <Step title="OpenBao (AWS STS live)">
    ...
    replacing the static [aws-vault](/security/tools/aws-vault) base key; other
    static service credentials continue moving into OpenBao KV.
  </Step>
