...
| `packages.agent-cli` | `agent run\|dispatch\|shell` — the single entry point that replaced the per-credential launcher wrappers in [nix-darwin](/nix/nix-darwin) |
...
A run is launched by the `agent` CLI:
...
| `--repo <owner/name>` | Write to exactly one repository, this run only | The launcher mints a per-run, single-repo-scoped GitHub App token via the write identity, behind a short claim-before-work lease so two runs never write the same repository. See [GitHub access](/autonomous-agents/github-access). |

The split is the point: a `--profile` run with no `--repo` can read its secrets
but can never push, and a `--repo` run's write reach is exactly one repository —
...
operational detail that lives in the internal ops runbooks, not in this public
overview.
